          ...
          <div className="order-1">
            <div className="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 p-6 md:p-8">
              <div className="mb-6">
                ...
                <p className="text-gray-500 text-sm">Fill out the form below and our representative will contact you shortly.</p>
              </div>
              <ContactForm />
            </div>
          </div>
          ...
          <div className="order-2 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {contactSections.map((item) => (
                <div key={item.title} className="flex items-start space-x-3 p-3 rounded-lg border border-gray-100 hover:border-red-100 hover:bg-red-50/30 transition-all group">
                  <div className={`p-2 rounded-md ${item.color} group-hover:scale-110 transition-transform`}>
                    ...
                    {item.items.map((detail, idx) => (
                      detail.href ? (
                        <a 
                          key={idx} 
                          href={detail.href}
                          className="block text-xs text-gray-600 hover:text-red-600 transition-colors"
                        >
                          {detail.label}
                        </a>
                      ) : (
                        <p key={idx} className="text-xs text-gray-600">{detail.label}</p>
                      )
                    ))}
                  </div>
                </div>
            ...
            {/* Map Section */}
            <div className="w-full h-48 sm:h-64 rounded-xl overflow-hidden shadow-sm border border-gray-100 mt-4">
              <iframe
